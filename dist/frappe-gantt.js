const k = "year", D = "month", $ = "day", E = "hour", Y = "minute", A = "second", L = "millisecond", S = {
  ...
  parse(n, t = "-", e = /[.:]/) {
    ...
    if (typeof n == "string") {
      let i, s;
      const r = n.split(" ");
      i = r[0].split(t).map((h) => parseInt(h, 10)), s = r[1] && r[1].split(e), i[1] = i[1] ? i[1] - 1 : 0;
      let o = i;
  ...
  format(n, t = "YYYY-MM-DD HH:mm:ss.SSS", e = "en") {
    const s = new Intl.DateTimeFormat(e, {
      month: "long"
    }).format(n), r = s.charAt(0).toUpperCase() + s.slice(1), o = this.get_date_values(n).map((g) => v(g, 2, 0)), h = {
      YYYY: o[0],
      MM: v(+o[1] + 1, 2, 0),
      DD: o[2],
      HH: o[3],
    ...
    let d = t;
    const p = [];
    return Object.keys(h).sort((g, u) => u.length - g.length).forEach((g) => {
      d.includes(g) && (d = d.replaceAll(g, `$${p.length}`), p.push(h[g]));
    }), p.forEach((g, u) => {
      d = d.replaceAll(`$${u}`, g);
    }), d;
  },
  diff(n, t, e = $) {
    let i, s, r, o, h, d, p;
    return i = n - t, s = i / 1e3, o = s / 60, r = o / 60, h = r / 24, d = h / 30, p = d / 12, e.endsWith("s") || (e += "s"), Math.floor(
      {
        milliseconds: i,
        seconds: s,
        minutes: o,
...
class O {
  ...
  draw_bar() {
    ...
    }), H(this.$bar, "width", 0, this.width), this.$bar.addEventListener("click", function() {
  ...
  draw_progress_bar() {
    ...
    const t = a.diff(this.task._start, this.gantt.gantt_start, "hour") / this.gantt.options.step * this.gantt.options.column_width;
  ...
  update_bar_position({ x: t = null, width: e = null }) {
    const i = this.$bar;
    if (t) {
      if (!this.task.dependencies.map((o) => this.gantt.get_bar(o).$bar.getX()).reduce((o, h) => t >= h, t)) {
        e = null;
        return;
      }
  ...
  update_label_position_on_horizontal_scroll({ x: t, sx: e }) {
    ...
    let h = this.$bar.getX() + this.$bar.getWidth(), d = s.getX() + t, p = r && r.getX() + t || 0, g = r && r.getBBox().width + 7 || 7, u = d + s.getBBox().width + 7, f = e + i.clientWidth / 2;
    s.classList.contains("big") || (u < h && t > 0 && u < f || d - g > this.$bar.getX() && t < 0 && u > f) && (s.setAttribute("x", d), r && (r.setAttribute("x", p), o.setAttribute("x", p)));
  }
  date_changed() {
    let t = !1;
  ...
  compute_start_end_date() {
    ...
    const r = t.getWidth() / this.gantt.options.column_width, o = a.add(
      i,
  ...
  compute_expected_progress() {
    this.expected_progress = a.diff(a.today(), this.task._start, "hour") / this.gantt.options.step, this.expected_progress = (this.expected_progress < this.duration ? this.expected_progress : this.duration) * 100 / this.duration;
  }
  compute_x() {
    const { step: t, column_width: e } = this.gantt.options, i = this.task._start, s = this.gantt.gantt_start;
    let o = a.diff(i, s, "hour") / t * e;
    this.gantt.view_is("Month") && (o = a.diff(i, s, "day") * e / 30), this.x = o;
  }
  compute_y() {
  ...
  compute_duration() {
    this.duration = a.diff(this.task._end, this.task._start, "hour") / this.gantt.options.step;
  }
  get_snap_position(t) {
    let e = t, i, s;
  ...
  update_label_position() {
    ...
    const h = i.getBBox().width, d = e.getWidth();
    h > d ? (i.classList.add("big"), s ? (s.setAttribute("x", e.getX() + e.getWidth() + r), t.setAttribute(
      "x",
      e.getX() + e.getWidth() + r
    ), i.setAttribute(
      "x",
...
class C {
  ...
  calculate_path() {
    ...
    const e = this.from_task.$bar.getY() + this.gantt.options.bar_height / 2, i = this.to_task.$bar.getX() - this.gantt.options.padding / 2 - 7, s = this.gantt.options.header_height + this.gantt.options.bar_height / 2 + (this.gantt.options.padding + this.gantt.options.bar_height) * this.to_task.task._index + this.gantt.options.padding, r = this.from_task.task._index > this.to_task.task._index, o = this.gantt.options.arrow_curve, h = r ? 1 : 0, d = r ? -o : o, p = r ? s + this.gantt.options.arrow_curve : s - this.gantt.options.arrow_curve;
    if (this.path = `
            M ${t} ${e}
            L ${t + 10} ${e}
            a ${o} ${o} 0 0 ${h ? 0 : 1} ${o} ${d}
    ...
    t > i) {
      ...
      const g = this.to_task.$bar.getY() + this.to_task.$bar.getHeight() / 2 - d, u = this.to_task.$bar.getX() - this.gantt.options.padding;
      this.path = `
                M ${t} ${e}
                L ${t + 10} ${e}
                a ${o} ${o} 0 0 ${h ? 0 : 1} ${o} ${d}
...
class B {
  ...
  setup_tasks(t) {
    this.tasks = t.map((e, i) => {
      if (e._start = a.parse(e.start), e.end === void 0 && e.duration !== void 0 && (e.end = e._start, e.duration.split(" ").forEach((h) => {
        let { duration: d, scale: p } = a.parse_duration(h);
        e.end = a.add(e.end, d, p);
      })), e._end = a.parse(e.end), a.diff(e._end, e._start, "year") < 0)
        throw Error(
          "start of task can't be after end of task: in task #, " + (i + 1)
        );
      if (a.diff(e._end, e._start, "year") > 10 && (e.end = null), e._index = i, !e.start && !e.end) {
        const o = a.today();
        e._start = o, e._end = a.add(o, 2, "day");
      }
      if (!e.start && e.end && (e._start = a.add(e._end, -2, "day")), e.start && !e.end && (e._end = a.add(e._start, 2, "day")), a.get_date_values(e._end).slice(3).every((o) => o === 0) && (e._end = a.add(e._end, 24, "hour")), (!e.start || !e.end) && (e.invalid = !0), typeof e.dependencies == "string" || !e.dependencies) {
        let o = [];
  ...
  setup_gantt_dates() {
    ...
    let t, e;
    this.gantt_start ? t = a.start_of(this.gantt_start, "day") : t = /* @__PURE__ */ new Date(), this.gantt_end ? e = a.start_of(this.gantt_end, "day") : e = /* @__PURE__ */ new Date();
    let i;
    for (let [h, d] of Object.entries(l))
      d === this.options.view_mode && (i = h);
    const [s, r] = this.options.view_mode_padding[i].map(a.parse_duration);
    t = a.add(
      t,
      -s.duration,
      s.scale
    ...
    let o;
    this.view_is(l.YEAR) ? o = "YYYY" : this.view_is(l.MONTH) ? o = "YYYY-MM" : this.view_is(l.DAY) ? o = "YYYY-MM-DD" : o = "YYYY-MM-DD HH", this.gantt_start = a.parse(
      a.format(t, o)
    ), this.gantt_start.setHours(0, 0, 0, 0), this.gantt_end = a.add(
      e,
      r.duration,
      r.scale
    );
  ...
  setup_date_values() {
    ...
    for (; t === null || t < this.gantt_end; )
      t ? this.view_is(l.YEAR) ? t = a.add(t, 1, "year") : this.view_is(l.MONTH) ? t = a.add(t, 1, "month") : t = a.add(
        t,
        this.options.step,
        "hour"
      ) : t = a.clone(this.gantt_start), this.dates.push(t);
  }
  bind_events() {
  ...
  update_button_position() {
    ...
    this.$side_header.style.left = i + this.$container.scrollLeft + r - this.$side_header.clientWidth + "px", this.$today_button.style.left = `${t.left + 20}px`;
  }
  make_grid_ticks() {
    ...
    if (this.options.lines !== "horizontal")
      for (let d of this.dates) {
        ...
        this.view_is(l.DAY) && d.getDate() === 1 && (p += " thick"), this.view_is(l.WEEK) && d.getDate() >= 1 && d.getDate() < 8 && (p += " thick"), this.view_is(l.MONTH) && d.getMonth() % 3 === 0 && (p += " thick"), c("path", {
          ...
          class: p,
          append_to: this.layers.grid
        }), this.view_is(l.MONTH) ? t += a.get_days_in_month(d) * this.options.column_width / 30 : t += this.options.column_width;
      }
  }
  highlightWeekends() {
    if (!(!this.view_is("Day") && !this.view_is("Half Day"))) {
      for (let t = new Date(this.gantt_start); t <= this.gantt_end; t.setDate(t.getDate() + 1))
        if (t.getDay() === 0 || t.getDay() === 6) {
          const e = a.diff(t, this.gantt_start, "hour") / this.options.step * this.options.column_width, i = (this.options.bar_height + this.options.padding) * this.tasks.length;
          c("rect", {
            x: e,
  ...
  computeGridHighlightDimensions(t) {
    ...
    if (this.view_is(l.DAY)) {
      let i = a.today();
      return {
        x: e + a.diff(i, this.gantt_start, "hour") / this.options.step * this.options.column_width,
        date: i
      };
    }
  ...
  make_grid_highlights() {
    if (this.options.highlight_weekend && this.highlightWeekends(), this.view_is(l.DAY) || this.view_is(l.WEEK) || this.view_is(l.MONTH) || this.view_is(l.YEAR)) {
      ...
      let r = document.getElementById(
        a.format(e).replaceAll(" ", "_")
      );
      r && r.classList.add("current-date-highlight");
    }
  }
  create_el({ left: t, top: e, width: i, height: s, id: r, classes: o, append_to: h }) {
    let d = document.createElement("div");
    return d.classList.add(o), d.style.top = e + "px", d.style.left = t + "px", r && (d.id = r), i ? d.style.width = s + "px" : d.style.width = "38px", s ? d.style.height = s + "px" : d.style.height = "30px", h.appendChild(d), d;
  }
  make_dates() {
  ...
  get_date_info(t, e) {
    let i = e ? e.date : a.add(t, 1, "day");
    const s = {
      Hour_lower: a.format(t, "HH", this.options.language),
      "Quarter Day_lower": a.format(
        t,
        "HH",
        this.options.language
      ),
      "Half Day_lower": a.format(
        t,
        "HH",
        this.options.language
      ),
      Day_lower: t.getDate() !== i.getDate() ? a.format(t, "D", this.options.language) : "",
      Week_lower: t.getMonth() !== i.getMonth() ? a.format(t, "D MMM", this.options.language) : a.format(t, "D", this.options.language),
      Month_lower: a.format(t, "MMMM", this.options.language),
      Year_lower: a.format(t, "YYYY", this.options.language),
      Hour_upper: t.getDate() !== i.getDate() ? a.format(t, "D MMMM", this.options.language) : "",
      "Quarter Day_upper": t.getDate() !== i.getDate() ? a.format(t, "D MMM", this.options.language) : "",
      "Half Day_upper": t.getDate() !== i.getDate() ? t.getMonth() !== i.getMonth() ? a.format(
        t,
        "D MMM",
        this.options.language
      ) : a.format(t, "D", this.options.language) : "",
      Day_upper: t.getMonth() !== i.getMonth() || !e ? a.format(t, "MMMM", this.options.language) : "",
      Week_upper: t.getMonth() !== i.getMonth() ? a.format(t, "MMMM", this.options.language) : "",
      Month_upper: t.getFullYear() !== i.getFullYear() ? a.format(t, "YYYY", this.options.language) : "",
      Year_upper: t.getFullYear() !== i.getFullYear() ? a.format(t, "YYYY", this.options.language) : ""
    };
    let r = this.view_is(l.MONTH) ? a.get_days_in_month(t) * this.options.column_width / 30 : this.options.column_width;
    const o = {
    ...
    return {
      date: t,
      formatted_date: a.format(t).replaceAll(" ", "_"),
      column_width: r,
      base_pos_x: o.x,
  ...
  set_scroll_position(t) {
    ...
    if (!e)
      return;
    const s = (a.diff(t, this.gantt_start, "hour") + 24) / this.options.step * this.options.column_width - this.options.column_width;
  ...
  bind_bar_events() {
    let t = !1, e = 0, i = 0, s = !1, r = !1, o = null, h = [];
    ...
    this.$svg.onclick = (p) => {
      ...
      h.forEach((w) => w.group.classList.remove("active")), g.classList.contains("left") ? s = !0 : g.classList.contains("right") ? r = !0 : g.classList.contains("bar-wrapper") && (t = !0), u.classList.add("active"), this.popup && this.popup.parent.classList.add("hidden"), this.popup && this.popup.parent.classList.add("hidden"), e = p.offsetX || p.layerX, p.offsetY || p.layerY, o = u.getAttribute("data-id"), h = [
        o,
      ...
      const f = [];
      let w;
      i && (w = p.currentTarget.scrollLeft - i);
      ...
      this.bar_being_dragged = null, h.forEach((g) => {
  ...
  bind_bar_progress() {
    ...
    _.on(this.$svg, "mousedown", ".handle.progress", (o, h) => {
      e = !0, t = o.offsetX || o.layerX, o.offsetY || o.layerY;
      const p = _.closest(".bar-wrapper", h).getAttribute("data-id");
      i = this.get_bar(p), s = i.$bar_progress, r = i.$bar, s.finaldx = 0, s.owidth = s.getWidth(), s.min_dx = -s.getWidth(), s.max_dx = r.getWidth() - s.getWidth();
    }), _.on(this.$svg, "mousemove", (o) => {
      if (!e)
        return;
      let h = (o.offsetX || o.layerX) - t;
      h > s.max_dx && (h = s.max_dx), h < s.min_dx && (h = s.min_dx);
      const d = i.$handle_progress;
      _.attr(s, "width", s.owidth + h), _.attr(d, "cx", s.getEndX()), s.finaldx = h;
